...
# 2. Load master data into the target database
mysql -h "${MYSQL_HOST}" -u "${MYSQL_USER}" -p"${MYSQL_PASSWORD}" catalogue < /schema/master-data.sql
if [ $? -ne 0 ]; then echo "Failed to load master-data.sql"; exit 1; fi
